...
#
# inputs:
#   - data_path: data root path with folders for complete audio file, segmented audio files, etc.
#   - sid2path_file: file with list of speaker-id to rel audio path mapping (generated by segment_audio_gigaspeech.py)
#   - out_filepath: manifest file used for training/validation in style-tts2 training 
#
...
def get_fid2meta(sid, sid2path, data_path):
    ...
    if cond1 and cond2 and cond3:
        ...
        cid, pid, aid = parts[1:4]
        audio_file = os.path.join(audio_path, cid, pid, f'{aid}.wav')
        ...
        meta_file = os.path.join(meta_path, cid, pid, f'{aid}.json')
        ...
        meta = {'title': audio_dict['title'], 'cid': cid, 'pid': pid, 'aid': aid, 'sid': sid, 'speaker': segment['speaker'],
...
if __name__ == '__main__':
    ...
    # args.data_path = os.path.join(work_dir, 'Datasets', 'GigaSpeech-Zhenhao')
    # args.sid2path_file = os.path.join(args.data_path, 'sid2path.txt')

    # # validation
    # args.sid_file = os.path.join(home_dir, 'code', 'repo', 'mqtts', 'datasets_ori', 'validation.txt')
    # args.out_filepath = os.path.join(work_dir, 'Data', 'GigaSpeech', 'val_list_nodur.txt')

    # # training
    # args.sid_file = os.path.join(home_dir, 'code', 'repo', 'mqtts', 'datasets_ori', 'training.txt')
    # args.out_filepath = os.path.join(work_dir, 'Data', 'GigaSpeech', 'train_list_nodur.txt')
    ...
    # end_time = time.time()
    # elapsed_time = end_time - start_time
    # # duration of single-thread processing: 39 seconds (val), 0.38 hrs (train, checking only, all generated already)
    # print('duration of processing fid2meta with single thread: {:.2f} seconds'.format(elapsed_time))

    # # get backup fid2meta (read from local, do not run if you have fid2meta already from previous run)
    ...
    # (take about 5 seconds for the validation data and 2 minutes for the training data)
    start_time = time.time()
    nprocs = psutil.cpu_count()
    manager = multiprocessing.Manager()
    return_dict = manager.dict()
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print('duration of processing manifest tuple with multithreads: {:.2f} seconds'.format(elapsed_time))

    # # get the manifest tuple list (with single thread, 23 min for the training data)
    # start_time = time.time()
    # tuple_list = get_manifest(fids_sel, fid2meta, use_spkr=False, use_dur=False)
    # end_time = time.time()
    # elapsed_time = end_time - start_time
    # print('duration of processing manifest tuple with single threads: {:.2f} seconds'.format(elapsed_time))
