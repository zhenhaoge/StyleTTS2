...
# to get started, the following files need to be prepared and copied from local to cluster
#   - original video file (possibally in lower resolution, such as 640X260, for faster processing)
#   - srt suttitle file with manual correction
# cluster data dir $ROOT_DIR/Datasets/YouTube/${account_id}/${recording_id}/${dur_id}
#
...
mkdir -p $OUT_DIR
VIDEO_FILE=$IN_DIR/${recording_id}_L1_640X360.mp4
VIDEO_FILE_ORI=$IN_DIR/${recording_id}_L1.mp4
if [ -f $VIDEO_FILE ] && [ ! -f $VIDEO_FILE_ORI ]; then
    echo "creating symbolic link: ${VIDEO_FILE} -> ${VIDEO_FILE_ORI}"
    ln -s $VIDEO_FILE $VIDEO_FILE_ORI    
fi
...
dur_lim_sec=$((tmp+1)) # get the ceiling value used in source seperation
...
AUDIO_FILE_DEFAULT=$IN_DIR/${basename}
mv $AUDIO_FILE_DEFAULT/accompaniment.wav" $AUDIO_FILE_ACC
mv $AUDIO_FILE_DEFUALT/vocals.wav" $AUDIO_FILE_VOC
[ -z "$( ls -A $AUDIO_FILE_DEFAULT )" ] && \
     echo "removing the default spleeter output dir: $AUDIO_FILE_DEFAULT ..." && \
     rm -rf $AUDIO_FILE_DEFAULT
cd $ROOT_DIR
...
top_db=30 # default 30 (20: more agressive, 40: more convservative)
out_ver=v1
...
in_txt_file=${meta_dir}/${recording_id}_L1_text_v3.txt
out_txt_file=${meta_dir}/${recording_id}_L2_text_v3.txt
source_lang='ZH'
target_lang='EN-US'
context_size=5
aip_key_file=$ROOT_DIR/Examples/deepl_api_keys.txt
account="gezhenhaonuaa@gmail.com"
python $WORK_DIR/helpers/translate_text.py \
    --in-txt-file ${in_txt_file} \
    --out-txt-file ${out_txt_file} \
...
L2_text_file=$OUT_DIR/meta/${recording_id}_L2_text_v3.txt # input 2: texts(L2)
L2_ts_file=$OUT_DIR/meta/${recording_id}_L2_ts-text_v3.csv # output: timestamps + texts(L2)
...
# # alternative: use another reference audio file from another recording
# recording_id_ext=20210915
# IN_DIR_EXT=${IN_DIR/${recording_id}/${recording_id_ext}}
# master_audio_file=$IN_DIR_EXT/${recording_id_ext}_L1_spk.wav
...
meta_file=$OUT_DIR/meta/${recording_id}_L2_ts-text_v3.csv # generated in step 5.2
...
r="1.0"
out_file=${out_dir}/${recording_id}_bg+L2_mono.wav
with_bg=true
...
mono_audio_file=${out_dir}/${recording_id}_bg+L2_mono.wav
ster_audio_file=${out_dir}/${recording_id}_bg+L2.wav
...
mp3_audio_file=${out_dir}/${recording_id}_bg+L2.mp3
